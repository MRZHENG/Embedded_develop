...
void myCanReceive(CanRxMsg*RxMessage){
   ...
   CAN_Receive(CAN1,CAN_FIFO0,RxMessage);
	 ...
	 // 判断是标准ID 还是扩展ID
	 if(RxMessage->IDE == CAN_Id_Standard && RxMessage->StdId == 0x100 &&RxMessage->RTR == CAN_RTR_Data){
	    OLED_ShowHexNum(2, 6, RxMessage->Data[0], 2);
		  OLED_ShowHexNum(2, 9, RxMessage->Data[1], 2);
			OLED_ShowHexNum(2, 12,RxMessage->Data[2], 2);
			OLED_ShowHexNum(2, 15,RxMessage->Data[3], 2);
	 }
}
